...
with open('word_tag_train.csv','r') as file:
...
with open('word_tag_train_freq.csv','w') as file:
...
with open('word_tag_test.csv','r') as file:
	reader=csv.DictReader(file)
	for row in reader:
		word_tag=row["Word_pos"]
		if word_tag not in word_tag_dict.keys():
...
with open('word_tag_test_freq.csv','w') as file:
	writer =csv.writer(file)
	writer.writerow(['Word_tag','Frequency'])
	for word,freq in word_tag_dict.items():
		writer.writerow([word,freq])